...
namespace
{
    ...
    TEST(rand_tests, float_test)
    {
        ...
        for(int i = 0; i < 500; i++)
        {
            auto val = util::get_random<float>(min, max);
            EXPECT_LT(val, max);
            EXPECT_GE(val, min);
        }
    }
    ...
    TEST(rand_tests, double_test)
    {
        ...
        for(int i = 0; i < 500; i++)
        {
            auto val = util::get_random<double>(min, max);
            EXPECT_LT(val, max);
            EXPECT_GE(val, min);
        }
    }
    ...
    TEST(rand_tests, uint8_test)
    {
        ...
        for(int i = 0; i < 500; i++)
        {
            auto val = util::get_random<uint8_t>(min, max);
            EXPECT_LE(val, max);
            EXPECT_GE(val, min);
        }
    }
    ...
    TEST(rand_tests, int64_test)
    {
        ...
        for(int i = 0; i < 500; i++)
        {
            auto val = util::get_random<int64_t>(min, max);
            EXPECT_LE(val, max);
            EXPECT_GE(val, min);
        }
    }
