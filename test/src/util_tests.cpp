...
TEST(rand_tests, float_test)
{
    ...
    for(int i = 0; i < 500; i++)
    {
        float val = util::get_random_real<float>(min, max);
        EXPECT_LT(val, max);
        EXPECT_GE(val, min);
    }
}
...
TEST(rand_tests, double_test)
{
    ...
    for(int i = 0; i < 500; i++)
    {
        double val = util::get_random_real<double>(min, max);
        EXPECT_LT(val, max);
        EXPECT_GE(val, min);
    }
}
...
TEST(rand_tests, uint8_test)
{
    ...
    for(int i = 0; i < 500; i++)
    {
        uint8_t val = util::get_random_int<uint8_t>(min, max);
        EXPECT_LE(val, max);
        EXPECT_GE(val, min);
    }
}
...
TEST(rand_tests, int64_test)
{
    ...
    for(int i = 0; i < 500; i++)
    {
        int64_t val = util::get_random_int<int64_t>(min, max);
        EXPECT_LE(val, max);
        EXPECT_GE(val, min);
    }
}